...
class LLMProvider(ABC):
    ...
    def _get_initial_prompt(self, readme_content: str) -> str:
        ...
        return f"""你是一个专业的开发环境配置助手。请根据GitHub项目的README文件，为用户生成详细的安装和配置命令序列。
                    ...
                    12. windows系统下，所有cd 命令之前，都需要再加上一个目录名字，例如你想进入d盘，请使用cd d: && d:,请注意是所有命令，包括类似 cd d: && d: && conda activate myenv && pip install 这样的命令都需要在前面加上cd d: && d:，请注意是所有命令，包括类似 cd d: && d: && conda activate myenv && pip install -r requirements.txt 这样的命令都需要在前面加上cd d: && d:
